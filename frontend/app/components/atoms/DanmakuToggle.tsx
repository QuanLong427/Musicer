...
export function DanmakuToggle() {
  const { enabled, hasDanmaku, error, toggleDanmaku, retryFetch } = useDanmaku();

  const tooltip = error ? "点击重试弹幕获取" : (hasDanmaku ? undefined : "当前音频无弹幕");

  return (
    <div className="relative ml-auto">
      <button
        onClick={error ? retryFetch : toggleDanmaku}
