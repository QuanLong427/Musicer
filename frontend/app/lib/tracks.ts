...
export async function scanTracks(): Promise<Track[]> {
  ...
  let dirs;
  try {
    dirs = await readdir(resolvedMusicDir, { withFileTypes: true });
  } catch {
    return tracks;
  }
  ...
  for (const dir of dirs) {
    ...
    let files;
    try {
      files = await readdir(path.join(resolvedMusicDir, subDir), { withFileTypes: true });
    } catch {
      continue;
    }
    ...
    for (const f of files) {
      ...
      const filePath = path.join(resolvedMusicDir, subDir, f.name);
      let size = 0;
      try {
...
export function resolveMusicPath(relativePath: string): string | null {
  const full = path.resolve(resolvedMusicDir, relativePath);
  if (!full.startsWith(resolvedMusicDir)) return null;
  return full;
}

export { resolvedMusicDir as MUSIC_DIR };
